...
describe('ApiAIAssistant', () => {
  ...
  it('AC-1 / spec 3,5,6: getSuggestedResponse forwards consumer actionTimeStamp into the request body', async () => {
    const consumerActionTimeStamp = 1777479641173;
    const fallbackActionTimeStamp = 1000;
    jest.spyOn(Date, 'now').mockReturnValue(fallbackActionTimeStamp);
    (mockWebex.request as jest.Mock).mockResolvedValue({body: {ok: true}});
    apiAIAssistant.setAIFeatureFlags({suggestedResponses: {enable: true}} as any);
    ...
    const result = await apiAIAssistant.getSuggestedResponse({
      agentId: 'test-agent-id',
      interactionId: 'interaction-1',
      actionTimeStamp: consumerActionTimeStamp,
    });
    ...
    expect(requestArgs.body.eventDetails.data.actionTimeStamp).toBe(
      String(consumerActionTimeStamp)
    );
    expect(requestArgs.body.eventDetails.data.actionTimeStamp).not.toBe(
  ...
  it('AC-1 / spec 7,9: getSuggestedResponse falls back to Date.now when actionTimeStamp is omitted', async () => {
    const fallbackActionTimeStamp = 1234567890;
    jest.spyOn(Date, 'now').mockReturnValue(fallbackActionTimeStamp);
    (mockWebex.request as jest.Mock).mockResolvedValue({body: {ok: true}});
    apiAIAssistant.setAIFeatureFlags({suggestedResponses: {enable: true}} as any);
